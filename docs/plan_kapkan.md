...
- [X] Создать SplashScreen для проверки токена
...
- [X] При старте Flutter-приложения загрузиться первым экраном.
- [X] Проверить, сохранён ли токен в SecureStorage.
- [X] Если токен есть — попытаться сделать запрос (например, GET /sessions) и убедиться, что токен валиден.
...
- [X] Наполнить таблицу recommendations 
- [X] Локализация RU/ENG
...
- [X] Добавитиь свагер документацию  
